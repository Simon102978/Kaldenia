...
{
	public class DoughExp : Item, IQuality
    {
		...
		[Constructable]
		public DoughExp() : base( 0x103d )
		{
			m_MinSkill = 0;
			m_MaxSkill = 10;
            Stackable = Core.ML;
            m_CookedFood = new BreadLoafExp();
		}
        ...
        public DoughExp( Serial serial ) : base( serial )
		{
		}
		...
		{
			private DoughExp m_Item;

			public InternalTarget(DoughExp item ) : base( 1, false, TargetFlags.None )
			{
				m_Item = item;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is JarHoney )
				{
					if(!((Item)targeted).Movable) return;
					if(((Item)targeted).Parent == null )
						new SweetDoughExp().MoveToWorld(((Item)targeted).Location, ((Item)targeted).Map );
					else
						from.AddToBackpack( new SweetDoughExp() );
					m_Item.Consume();
					((Item)targeted).Consume();
