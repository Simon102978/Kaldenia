...
namespace Server.Items.Crops
{
	public class HoneydewMelonSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( ( cropsnear.Count > 1 ) ) { from.SendMessage( "Il y a trop de plants à proximit??#$?&*." ); return; }
	...
	public class HoneydewMelonCrop : BaseCrop
	{
		...
		{
			plant.PickGraphic = ( 3164 );
			plant.FullGraphic = ( 3164 );
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				{
					if ( m_yield < 1 ) { from.SendMessage( "Il n'y a rien à r??#$?&*colter ici." ); }
					else
					{
				...
				else { from.SendMessage( "Vous êtes trop loin pour r??#$?&*colter quelque chose." ); }
			}
		}
