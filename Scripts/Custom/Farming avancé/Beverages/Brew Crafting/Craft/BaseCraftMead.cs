...
namespace Server.Items
{
	public abstract class BaseCraftMead : Item, ICraftable
	{
		...
		public BaseCraftMead( Serial serial ) : base( serial ) { }
		...
		public BaseCraftMead( int itemID ) :  base( itemID )
		{
		...
		public override void AddNameProperties( ObjectPropertyList list )
 		{
			...
			if ( m_Quality == MeadQuality.Exceptional ) list.Add( 1060847, "Black Label\t{0} Mead", meadType );
		...
		public override void OnAosSingleClick( Mobile from )
		{
			...
			if (this.Name == null )
			{
				...
				else this.LabelTo( from, "Mead" );
			}
			else this.LabelTo( from, "{0}", this.Name );
			meadType = BrewingResources.GetName( m_Variety );
			if ( m_Quality == MeadQuality.Exceptional ) this.LabelTo( from, "Black Label {0} Mead", meadType );
