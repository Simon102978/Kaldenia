...
namespace Server.Gumps
{
    public class ConfirmGump : Gump
    {
        ...
        public ConfirmGump(Mobile from, CustomPlayerMobile mobile, CarnetAdresse carnet) : base(0, 0)
        {
            ...
            AddLabel(28, 31, 2101, String.Format("D??#$?&*sirez-vous que {0} vous ajoute à", m_From.Name));
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (info.ButtonID == 0)
            {
                m_From.SendMessage(String.Format("{0} a refus??#$?&* d'être ajout??#$?&* à votre carnet d'adresse.", m_Mobile.Name));
            }
            else
            {
                ...
                {
                    m_From.SendMessage(String.Format("{0} a accept??#$?&* d'être ajout??#$?&* à votre carnet d'adresse.", m_Mobile.Name));
