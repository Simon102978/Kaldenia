...
namespace Server.Items
{
    public abstract class BaseCraftAle : Item, ICraftable
	{
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public AleQuality Quality { get{ return m_Quality; } set{ m_Quality = value; InvalidateProperties(); } }
		...
		public override void OnAosSingleClick( Mobile from )
		{
            ...
            if (m_Quality == AleQuality.Exceptional)
            {
                LabelTo(from, "Black Label {0} Ale", aleType);
            }
            else
            {
                LabelTo(from, "{0}", aleType);
            }
		}
        ...
        public virtual int OnCraft(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, ITool tool, CraftItem craftItem, int resHue)
        {
            ...
            if (Quality == AleQuality.Exceptional)
            {
                Crafter = from;
            }
