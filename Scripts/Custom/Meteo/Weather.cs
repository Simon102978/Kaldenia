    ...
    public class Weather
    {
        ...
        public virtual Temperature AdjustTemperatureBySeason(Temperature t)
        {
            ...
            catch (Exception e)
            {
                if (entry == null)
                {
                    Misc.ExceptionLogging.WriteLine(e, "entry was null");
                    return t;
                }
                else
                {
                    Misc.ExceptionLogging.WriteLine(e, "Le length de entry est de {0} alors que l'index de temp??#$?&*rature" +
                       " ??#$?&*tait {1} ce qui donne un chiffre de {2}. La saison ??#$?&*tait {3}.", entry.Length, t, (int)t, season);
