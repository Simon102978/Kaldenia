...
{
    public class DegGrandeurGump : BaseProjectMGump
	{
		...
		public DegGrandeurGump(CustomPlayerMobile from, Deguisement deg,    int page = 0)
            : base("Grandeur", 200, 200, true)
        {
			...
			if (m_Deg.Race != null)
			{
				...
				while (n <= (int)m_Deg.Race.GrandeurMax)
				{
				...
				if (((int)m_Deg.Race.GrandeurMax - (int)m_Deg.Race.GrandeurMin) > (page + 1) * 9)
				{
		...
		public override void OnResponse(NetState sender, RelayInfo info)
        {
			     ...
			     switch (info.ButtonID)
				 {
					 ...
					 case 1:
						 {
							m_From.SendGump(new DegGrandeurGump(m_From,m_Deg, m_Page - 1));
							 break;
						 }
					 case 2:
						 {
			...
			{
				m_Deg.Grandeur = (GrandeurEnum)(info.ButtonID - 100);
				m_From.SendGump(new CustomDisguiseGump(m_From, m_Deg));
			}
		}
	}
