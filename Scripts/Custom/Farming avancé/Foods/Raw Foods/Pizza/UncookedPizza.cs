...
namespace Server.Items
{
	public class UncookedPizza : Item
	{
		...
		private BaseFood m_CookedFood{ get{ return new Pizza( this.Desc, this.Hue ); } }
		...
		public class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is BrightEggsExp || targeted is EasterEggs )
				{
					if(!((Item)targeted).Movable) return;
...
catch
{
			...
			private class InternalTimer : Timer
			{
				...
				public InternalTimer( Mobile from, IPoint3D p, Map map, int min, int max, BaseFood cookedFood ) : base( TimeSpan.FromSeconds( 3.0 ) )
				{
					m_From = from;
					m_Point = p;
					m_Map = map;
